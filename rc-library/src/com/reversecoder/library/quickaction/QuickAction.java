package com.reversecoder.library.quickaction;
...
public class QuickAction extends PopupWindows implements OnDismissListener {
		...
		if (actionItems != null) {
			for (ActionItem actionItem : actionItems) {
				if (actionItem.getActionId() == actionId) {
					Log.d("ActionItem", actionItem.getTitle());
					return actionItem;
				}
			}
		}
	...
	public int getActionItemIndexByID(int actionId) {

		if (actionItems != null) {
			for (int i = 0; i < actionItems.size(); i++) {
				if (actionItems.get(i).getActionId() == actionId) {
					Log.d("ActionItem", 1 + "");
					return i;
				}
			}
	...
	public void addActionItem(ActionItem action) {
		...
		if (mOrientation == HORIZONTAL) {
			container = mInflater.inflate(R.layout.quickaction_item_horizontal,
					null);
			// AllSettingsManager.applyFont(mContext, container, mContext
			// .getResources().getString(R.string.font_Raleway_SemiBold));
//			TypefaceHelper.typeface(container);
		} else {
			container = mInflater.inflate(R.layout.quickaction_item_vertical,
					null);
			// AllSettingsManager.applyFont(mContext, container, mContext
			// .getResources().getString(R.string.font_Raleway_SemiBold));
//			TypefaceHelper.typeface(container);
		...
		container.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!getActionItemByID(actionId).isSticky()) {
					mDidAction = true;
		...
		if (mOrientation == HORIZONTAL && mChildPos != 0) {
			View separator = mInflater.inflate(
					R.layout.quickaction_horiz_separator, null);
	...
	public void removeActionItem(ActionItem action, int itemPosition) {
		actionItems.remove(action);
		mTrack.removeViewAt(itemPosition);
	}
