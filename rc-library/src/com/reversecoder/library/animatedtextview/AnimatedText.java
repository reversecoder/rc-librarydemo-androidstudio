...
@SuppressLint("NewApi")
public abstract class AnimatedText implements IAnimatedText {
	...
	protected AnimatedTextView mHTextView;

	@Override
	public void init(AnimatedTextView hTextView, AttributeSet attrs, int defStyle) {
	...
	private void prepareAnimate() {
		...
		oldStartX = (mHTextView.getMeasuredWidth() - mHTextView.getCompoundPaddingLeft() - mHTextView.getPaddingLeft()
				- mOldPaint.measureText(mOldText.toString())) / 2f;
		startX = (mHTextView.getMeasuredWidth() - mHTextView.getCompoundPaddingLeft() - mHTextView.getPaddingLeft()
				- mPaint.measureText(mText.toString())) / 2f;
