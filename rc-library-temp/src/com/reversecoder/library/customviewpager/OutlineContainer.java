package com.reversecoder.library.customviewpager;
...
public class OutlineContainer extends FrameLayout implements Animatable {
	...
	private void init() {
		...
		mOutlinePaint.setStrokeWidth(AllSettingsManager.dpToPx(getResources(),
				2));
		mOutlinePaint.setColor(getResources().getColor(R.color.holo_blue));
		mOutlinePaint.setStyle(Style.STROKE);

		int padding = AllSettingsManager.dpToPx(getResources(), 10);
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		int offset = AllSettingsManager.dpToPx(getResources(), 5);
		if (mOutlinePaint.getColor() != CustomViewPager.sOutlineColor) {
			mOutlinePaint.setColor(CustomViewPager.sOutlineColor);
		}
