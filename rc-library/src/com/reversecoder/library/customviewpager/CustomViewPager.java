...
public class CustomViewPager extends ViewPager {
	...
	public CustomViewPager(Context context, AttributeSet attrs) {
		...
		int effect = ta.getInt(R.styleable.CustomViewPager_cv_style, 0);
		String[] transitions = getResources().getStringArray(
				R.array.custom_effects_viewpager);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.CustomViewPager_fadeEnabled,
				false));
		setOutlineEnabled(ta.getBoolean(
				R.styleable.CustomViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.CustomViewPager_outlineColor,
				Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
