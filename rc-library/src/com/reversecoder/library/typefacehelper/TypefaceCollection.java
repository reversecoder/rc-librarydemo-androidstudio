...
public class TypefaceCollection {
	...
	public static class Builder {
		...
		/**
		 * This typeface is set in fist
		 * {@link #set(int, Typeface)} call and is used for all
		 * unset styles in {@link #create()} call
		 */
		 ...
		 *             when invalid typefaceStyle is passed
		 * @return self
		 */
		...
		/**
		 * Creates typeface collection. If not all styles are set, uses fist
		 * typeface that has been set for all unset styles.
		 * 
		 * @throws IllegalStateException
		 *             when no {@link Typeface} has been set
		 *             via {@link #set(int, Typeface)}
		 * @return TypefaceCollection
		 */
		public TypefaceCollection create() {
			if (mDefaultTypeface == null) {
				throw new IllegalStateException(
						"At least one typeface style have to be set!");
			}
			...
			if (mCollection.mTypefaces.get(Typeface.BOLD_ITALIC) == null) {
				mCollection.mTypefaces.put(Typeface.BOLD_ITALIC,
						mDefaultTypeface);
			}
	...
	public static TypefaceCollection createSystemDefault() {
		return new Builder().set(Typeface.NORMAL, Typeface.DEFAULT)
				.set(Typeface.BOLD, Typeface.DEFAULT_BOLD)
				.set(Typeface.ITALIC, Typeface.DEFAULT)
				.set(Typeface.BOLD_ITALIC, Typeface.DEFAULT_BOLD).create();
	}
	...
	private static void validateTypefaceStyle(int typefaceStyle) {
		...
		default:
			throw new IllegalArgumentException(
					"Invalid typeface style! Have to be one of "
